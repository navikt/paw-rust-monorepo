...
pub(crate) struct ArbeidssoekerRowV2 {
    pub id: i64,
...
async fn select_by_identitetsnummer_asc(
    ...
    let rows = sqlx::query_as::<_, ArbeidssoekerRowV2>(
        r#"
        SELECT
            a.id,
            a.arbeidssoeker_id,
...
pub async fn insert(
    tx: &mut Transaction<'_, Postgres>,
    row: &ArbeidssoekerRowV2,
) -> anyhow::Result<i64> {
    let id = sqlx::query_scalar(
        r#"
        INSERT INTO arbeidssoekere_v2 (
            arbeidssoeker_id,
            identitetsnummer,
            fornavn,
            mellomnavn,
