...
mod tests {
    ...
    use test_data_generator::eksterne_hendelser::create_dummy_bekreftelse;
    ...
    struct TestContext {
        ...
        processor: BekreftelseProcessor,
    }
    ...
    async fn init() -> &'static TestContext {
        INIT.get_or_init(|| async {
            ...
            TestContext {
                ...
                processor: BekreftelseProcessor::new(schema_registry_settings.clone()),
            }
        })
        .await
    }
    ...
    async fn test_process_messages() {
        ...
        let bekreftelse = create_dummy_bekreftelse("01017012345", periode_id, bekreftelse_id);
        let payload = context
            .create_avro_payload(PAW_BEKREFTELSE_TOPIC, bekreftelse.clone())
            .await;
        ...
        assert_eq!(row.id, bekreftelse.id);
        assert_eq!(row.periode_id, bekreftelse.periode_id);
        assert_eq!(
            row.bekreftelsesloesning,
            bekreftelse.bekreftelsesloesning.as_ref().to_string()
        );
    }
}
