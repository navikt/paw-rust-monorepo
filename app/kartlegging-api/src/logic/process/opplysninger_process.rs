...
impl PayloadProcessor for OpplysningerProcessor {
    async fn process_payload<'a>(
        ...
        match message.payload() {
            ...
            Some(payload) => {
                let hendelse: Opplysninger = self
                    .deserializer
                    .deserialize(payload)
                    .await
...
mod tests {
    ...
    use crate::model::dao::opplysninger;
    use eksterne_hendelser::vo::jobbsituasjon::Beskrivelse;
    ...
    async fn test_process_messages() {
        ...
        let message = context
            .avro_generator
            .create_avro_message("paw.opplysninger-om-arbeidssoeker-v1", opplysninger)
            .await;
        ...
        assert_eq!(row.id, opplysninger_id);
        assert_eq!(row.periode_id, periode_id);
        assert_eq!(
            row.jobbsituasjon,
            vec![Beskrivelse::HarBlittSagtOpp.as_ref().to_string()]
        );
    }
