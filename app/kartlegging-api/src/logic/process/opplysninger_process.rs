use crate::logic::mutation::opplysninger_mutation;
...
pub struct OpplysningerProcessor {
    pub deserializer: AvroDeserializer,
}

impl OpplysningerProcessor {
    ...
    pub async fn process_payload<'a>(
        ...
        let hendelse: Opplysninger = self.deserializer.deserialize(payload).await.map_err(|e| {
            ProcessorError::from(format!("Failed to deserialize payload: {}", e.to_string()))
        })?;

        tracing::info!("Mottok hendelse: {:?}", &hendelse);

        opplysninger_mutation::lagre_hendelse(tx, &hendelse).await?;
...
mod tests {
    ...
    struct TestContext {
        ...
        processor: OpplysningerProcessor,
    }
    ...
    async fn test_process_messages() {
        ...
        let opplysninger = create_dummy_opplysninger("01017012345", periode_id, opplysninger_id);
        let payload = context
            .create_avro_payload(PAW_OPPLYSNINGER_TOPIC, opplysninger.clone())
            .await;
        ...
        let mut tx = context.start_tx().await;
        let optional_row = opplysninger::select_by_id(&mut tx, &opplysninger_id)
            .await
            .expect("Kunne ikke hente opplysninger");
        tx.commit().await.expect("Kunne ikke commit transaksjon");

        assert!(optional_row.is_some());
        let row = optional_row.expect("Ingen opplysninger funnet");
        assert_eq!(row.id, opplysninger.id);
        assert_eq!(row.periode_id, opplysninger.periode_id);
        assert_eq!(
            row.jobbsituasjon,
            opplysninger
                .jobbsituasjon
