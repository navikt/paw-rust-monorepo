...
mod tests {
    ...
    use test_data_generator::eksterne_hendelser::create_dummy_profilering;
    ...
    struct TestContext {
        ...
        processor: ProfileringProcessor,
    }
    ...
    async fn init() -> &'static TestContext {
        INIT.get_or_init(|| async {
            ...
            TestContext {
                ...
                processor: ProfileringProcessor::new(schema_registry_settings.clone()),
            }
        })
        .await
    }
    ...
    async fn test_process_messages() {
        ...
        let payload = context
            .create_avro_payload(PAW_PROFILERING_TOPIC, profilering.clone())
            .await;
        ...
        assert_eq!(row.id, profilering.id);
        assert_eq!(row.periode_id, profilering.periode_id);
        assert_eq!(
            row.opplysninger_id,
            profilering.opplysninger_om_arbeidssoker_id
        );
        assert_eq!(
            row.profilert_til,
            profilering.profilert_til.as_ref().to_string()
        );
    }
}
