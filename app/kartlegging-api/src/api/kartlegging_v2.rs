...
use crate::model::dto::response_v2::KartleggingV2Response;
...
pub const API_KARTLEGGING_PATH: &str = "/api/v2/kartlegging";
...
async fn finn_kartlegging(
    ...
    request: String,
) -> Result<Json<KartleggingV2Response>, ProblemDetails> {
    ...
    let response = match query_request {
        QueryRequest::Identitetsnummer(query) => {
            ...
            query::arbeidssoeker_v2_query::finn_for_identitetsnummer_query_request(&mut tx, &query)
                .await
                .map_err(|e| {
        ...
        QueryRequest::TilknyttetKontor(query) => {
            query.validate(API_KARTLEGGING_PATH)?;
            query::arbeidssoeker_v2_query::finn_for_kontortilknytning_query_request(&mut tx, &query)
                .await
                .map_err(|e| {
                    tracing::error!("Feil ved spørring: {}", e);
                    ProblemDetails::database_error(API_KARTLEGGING_PATH, "Spørring feilet")
