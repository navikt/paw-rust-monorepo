...
mod tests {
    ...
    async fn test_irrelevante_meldinger_ignoreres() -> Result<()> {
        ...
        assert!(oppdater_ferdigstilte_oppgaver(&ugyldig_message, &mut tx).await.is_ok());
        tx.commit().await?;

        let irrelevant_message = lag_kafka_melding(OPPGAVE_OPPRETTET_JSON.as_bytes().to_vec());
        let mut tx = pg_pool.begin().await?;
        assert!(oppdater_ferdigstilte_oppgaver(&irrelevant_message, &mut tx).await.is_ok());
        tx.commit().await?;

        let ukjent_message = lag_kafka_melding(OPPGAVE_FERDIGSTILT_JSON.as_bytes().to_vec());
        let mut tx = pg_pool.begin().await?;
        assert!(oppdater_ferdigstilte_oppgaver(&ukjent_message, &mut tx).await.is_ok());
    ...
    async fn test_ferdigstilt_oppgave() -> Result<()> {
        ...
        let mut tx = pg_pool.begin().await?;
        oppdater_ferdigstilte_oppgaver(&message, &mut tx).await?;
        tx.commit().await?;
