...
pub async fn process_hendelselogg_message(
    ...
    if avvist::er_avvist_hendelse_under_18(hendelse_type, &opplysninger) {
        avvist::opprett_oppgave_for_avvist_hendelse(json, app_config, tx).await?;
    } else if hendelse_type == interne_hendelser::STARTET_HENDELSE_TYPE
        && startet::er_startet_eu_eoes_ikke_bosatt(&opplysninger)
    {
        startet::opprett_oppgave_for_startet_hendelse(json, tx).await?;
    }
