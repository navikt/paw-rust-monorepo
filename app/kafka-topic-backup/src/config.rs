...
pub fn read_app_config() -> Result<AppConfig, Box<dyn Error>> {
    let file_content = match std::env::var("NAIS_CLUSTER_NAME") {
        Ok(_) => include_str!("../config/nais/app_config.toml"),
        Err(_) => include_str!("../config/local/app_config.toml"),
    };
    let config: AppConfig = toml::from_str(file_content)?;
    Ok(config)
}
