...
pub async fn hwm_process_message(
    ...
    if hwm_ok {
        ...
        increment_kafka_messages_processed(
            true,
            &topic.to_string(),
            msg.partition(),
            res.is_err()
...
pub fn increment_kafka_messages_processed(above_hwm: bool, topic: &String, partition: i32, error: bool) {
    ...
    counter_vec
        .with_label_values(&[&above_hwm.to_string(), topic, &partition.to_string(), &error.to_string()])
        .inc();
}
