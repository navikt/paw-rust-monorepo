...
pub async fn skriv_perioder(
    ...
    let mut builder = sqlx::QueryBuilder::new(
        "INSERT INTO perioder (id, arbeidssoeker_id, trenger_kontroll, sist_oppdatert) ",
    );
...
pub async fn hent_perioder(
    ...
    sqlx::query_as::<_, PeriodeRad>(
        "SELECT id, arbeidssoeker_id, trenger_kontroll, sist_oppdatert
         FROM perioder
         WHERE id = ANY($1)",
    )
    .bind(uuid_liste)
...
pub async fn hent_perioder_eldre_enn(
    ...
    sqlx::query_as::<_, PeriodeRad>(
        "SELECT id, arbeidssoeker_id, trenger_kontroll, sist_oppdatert
         FROM perioder
         WHERE trenger_kontroll = false AND sist_oppdatert < $1
