...
pub async fn skriv_periode_data(
    ...
    sqlx::query(
        r#"INSERT INTO perioder (id, arbeidssoeker_id, identitetsnummer, stoppet, sist_oppdatert, trenger_kontroll, siste_kontroll_tidspunkt, tilstand, bekreftet)
           ...
           ON CONFLICT (id) DO UPDATE SET
             ...
             bekreftet = perioder.bekreftet OR EXCLUDED.bekreftet"#,
    )
    .bind(id)
...
pub async fn skriv_startet_hendelse(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    startet: Startet,
) -> Result<(), sqlx::Error> {
    ...
    let sist_oppdatert = startet.metadata.tidspunkt;
...
pub async fn skriv_periode_melding(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    periode: Periode,
) -> Result<(), sqlx::Error> {
    ...
    let stoppet: Option<serde_json::Value> = periode
        ...
        .map(|avsluttet| Stoppet {
            ...
            utfoert_av: avsluttet.utfoert_av.bruker_type,
        })
